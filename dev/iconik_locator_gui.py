...
class LocatorApp(tk.Tk):
    def __init__(self) -> None:
        ...
        self.share_multi_var = tk.StringVar(value=str(cfg.get("multi_share") or cfg.get("multi") or "ERROR"))
        self.file_multi_var = tk.StringVar(value=str(cfg.get("file_multi") or "ALL"))
    ...
    def _build_ui(self) -> None:
        ...
        for idx in range(8):
            options.columnconfigure(idx, weight=1 if idx in (1, 3, 5) else 0)
        ...
        ttk.Label(options, text="Share links").grid(row=0, column=2, sticky="w", padx=(18, 8))
        ttk.Combobox(options, textvariable=self.share_multi_var, values=loc.VALID_MULTI, state="readonly", width=10).grid(row=0, column=3, sticky="w")
        ttk.Label(options, text="File copies").grid(row=0, column=4, sticky="w", padx=(18, 8))
        ttk.Combobox(options, textvariable=self.file_multi_var, values=loc.VALID_MULTI, state="readonly", width=10).grid(row=0, column=5, sticky="w")
        ttk.Checkbutton(options, text="Save settings", variable=self.save_var).grid(row=0, column=6, sticky="e", padx=(18, 0))

        input_box = ttk.LabelFrame(outer, text="Lookup", padding=12)
        input_box.grid(row=2, column=0, sticky="ew", pady=(12, 0))
        ...
        self.output_text.grid(row=4, column=0, sticky="nsew", pady=(12, 0))
    ...
    def _lookup_worker(self, target: str) -> None:
        try:
            ...
            share_multi = loc.normalize_mode(self.share_multi_var.get(), "ERROR", loc.VALID_MULTI)
            file_multi = loc.normalize_mode(self.file_multi_var.get(), "ALL", loc.VALID_MULTI)
